...
giles.boxplots.2 <- function (graphs, ws) {
 # graphs: c(1000,835,715,625,555,525)
  ...
  args <- unlist(lapply(graphs, function(x) lapply(ws,
...
draw.twelve <- function (file) {
  ...
  par(oma=c(3, 4, 0, 0), mar=rep(2, 4), cex=.7)
  draw.three (paste (file, "rds-2.out", sep="-"), "RDS", TRUE)
  draw.three (paste (file, "dag-2.out", sep="-"), "DAG")
  draw.three (paste (file, "rep-2.out", sep="-"), "REP")
  draw.three (paste (file, "rep-1.out", sep="-"), "MCMC")
  dev.off ()
}
...
get.tests <- function (net.size, w, name) {
	...
	m <- lapply(c(1:100), function (x) simulate(modl))
...
get.more.tests <- function (net.size, w, name) {
	modl <- get.graph.model (net.size, w)
	f <- file (paste(name, "graph", sep="."), open="w")
	m <- lapply(c(1:100), function (x) simulate(modl))
	lapply(m, function (x) writethis(x,f))
	writeLines (text= "size 0", con = f)
	close(f)
	save (m, file=paste(name, "Rdata", sep="."))
}
...
writethis <- function (x, f) {
	writeLines (text= paste("size", format(network.edgecount(x)), sep=" "), con = f)
	write.table(as.matrix.network(x, matrix.type="edgelist"), sep = " ", file =f, row.names=FALSE, col.names=FALSE)
	writeLines (text= paste("infection", format(network.size(x)), sep=" "), con = f)
	write.table(x %v% "infected", sep = " ", file =f, row.names=FALSE, col.names=FALSE)
}
